...
export default async function MoviePage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center py-12 px-4">
      <div className="max-w-3xl w-full bg-gray-900 rounded-lg shadow-lg flex flex-col md:flex-row overflow-hidden">
        <img
          src={movie.poster}
          alt={movie.title}
        ...
        <div className="flex-1 p-8 flex flex-col">
          ...
          <p className="text-gray-200 mt-4">{movie.synopsis}</p>
        </div>
      </div>
    </div>
  );
